...
func _init() -> void:
	...
	# Default RDSamplerState gives nearest-neighbour + clamp, which is correct for pixel-art reads
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	# The shader reconstructs view-space depth from NDC, so it needs the inverse projection matrix
	...
	# Pad to vec4 alignment required by std430/push_constant layout rules
