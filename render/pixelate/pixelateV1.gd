...
@export_enum("resurrect_8", "resurrect_64", "aap_64", "mushroom", "citrink", "blk_nx64", "db32") var palette: String = "resurrect_8":
	set(v):
		palette = v
		_queue_palette_upload(PALETTES[v])
...
func _init() -> void:
	...
	_queue_palette_upload(PALETTES[palette])
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	# Pack push constants: raster_size (vec2) + palette_size (int) + pad (int)
	var push_constants := PackedByteArray()
	push_constants.resize(16)
	push_constants.encode_float(0, size.x)
	push_constants.encode_float(4, size.y)
	push_constants.encode_s32(8,  PALETTES[palette].size())
	push_constants.encode_s32(12, 1 if use_lab else 0)
